  ...
  <div>
    ...
    <div class="bg-white border border-stone-200 rounded-xl p-6 mb-6">
      ...
      >
        ...
        <p class="text-stone-600 text-sm">Drag & drop PDF quotations here, or <span class="text-green-600 font-medium">click to browse</span></p>
        <p class="text-stone-400 text-xs mt-1">Upload 2-10 PDF files from different suppliers</p>
    ...
    <div v-if="result">
      ...
      <p class="text-xs text-stone-400 mt-3 text-right">
...
function handleDrop(e: DragEvent) {
  ...
  if (!dropped.length) { toast.error('Only PDF files are accepted'); return }
  files.value.push(...dropped)
}

function handleFileSelect(e: Event) {
  const input = e.target as HTMLInputElement
  const selected = Array.from(input.files || []).filter((f) => f.type === 'application/pdf')
  if (!selected.length) { toast.error('Only PDF files are accepted'); return }
