<template>
  <div>
    <div class="mb-6">
      <p class="text-xs text-stone-400 mb-1">Report &gt; Supply Return Summary</p>
      <h2 class="text-lg font-semibold text-stone-900">Supply Return Summary</h2>
    </div>
    ...
    <div v-else class="overflow-x-auto bg-white border border-stone-200 rounded-xl">
      <table class="w-full text-sm text-left">
        ...
        <tbody v-else-if="!rows.length">
          <tr>
            <td colspan="6" class="px-4 py-8 text-center text-stone-400">No supply return records found for the selected criteria.</td>
          </tr>
        </tbody>
        <tbody v-else>
...
async function generate() {
  ...
  try {
    const { data } = await api.get('/reports/supply-return-summary', {
      params: { from: from.value, to: to.value, search: search.value || undefined },
    })
    rows.value = data.data || []
  } catch {
    toast.error('Failed to load supply return summary')
  } finally {
    loading.value = false
  }
}
